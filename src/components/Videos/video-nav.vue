...
<template>
    <div class="video-nav">
        <h1>Search</h1>
        <game-search v-model="video.game.title" @update:game="setGame($event)" />
        <player-search v-model="video.player1.name" @update:player="setPlayer1($event)" />
        <character-search
        ...
        />
        <tag-search v-model="video.tags" :taggable="true" @update:tags="setTags($event)" />
