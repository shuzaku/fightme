        ...
        <div class="table-container" v-if="showCharacterList">
          <table>
            <tr v-for="character in characters" :key="character" class="character-row">
              <td><div class="image-container" :style="{ background: 'url(' + character.ImageUrl + ') no-repeat top center/cover'}" ></div> {{character.Name}} <button @click="removeCharacter(character)" class="deleteBtn"> X </button></td>
            </tr>
            <tr>
              <textarea type="textarea" name="title" placeholder="Add Characters (separate by commas)" v-model="newCharacters" />
            </tr>
          </table>
        </div>

        <div v-if="title !== oldTitle || removedCharacters.length >= 1 || newCharactersList " class="changes-container">
          <h3>Changes</h3>
          ...
          <div v-if="removedCharacters.length >= 1"> 
            <p>Removed Characters</p>
              <ul>
                <li v-for="character in removedCharacters" :key="character">{{character}}</li>
              </ul>
          </div>
          <div v-if="newCharactersList"> 
            <p>Added Characters</p>
              <ul>
                <li v-for="character in newCharactersList" :key="character">{{character}}</li>
              </ul>
          </div>
        </div>
